...
/* know is the function to start the program and enter inputs according to the given questions, this input will then call the function suggest which will then call
*/

know() :-
    write('Welcome to course advisory system. '),
    nl,
    write('Enter the following data about the IIIT-ian, in prescribed format. '),
    nl,
    write('Out of these 3 which are you most interested in:'),
    nl,
    write('1) cse'),
    nl,
    write('2) ece'),
    ...
    read(A),
    nl,
    write('What is the average grade you got in it? Enter the no. for example 8.4 '),
    nl,
    read(B),
    nl,
    write('Did you do a BTech Project in this domain? y-yes, n-no'),
    nl,
    read(C),
    nl,
    suggest([A,B,C]).

/*suggest is a function to call other functions which add the available courses to the final list which will be printed

*/

suggest([A,B,C]):-
    difficulty_level(A,B,C),
    write('All the best for your future.').

/* difficulty level is to decide that student should take difficult or medium or easy courses depending upon btp and cgpa (determined by certain heuristics */

difficulty_level(A,B,_):- B>=8, get_difficult(A),!.
difficulty_level(A,B,C):- B>=6, C=y, get_medium(A),!.
difficulty_level(A,_,_):- get_easy(A),!.
